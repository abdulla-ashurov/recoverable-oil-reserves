      subroutine ndoipi(ni,nj,nk,npl,ro,socr,b0,drtcr,oiip,nvar,ntime,ntstep,nt)                                              
! Расчет текущих недренируемых геологических запасов, плотности текущих недренируемых геологических запасов  
      character*3 ant,cha 
      integer*4 nt        
                                                                                                         
      integer*2 npl(166),ntstep(100)                                                                                 
      real*4 a(248),b(248),c(248)                                                                        
...
!              else
        ...
        enddo                                                                                            
        write(3,rec=j) (c(i),i=1,ni)                                                                   
! Недренируемый объем нефти в ячейках в пластовых условиях,м3
...
! ndopi1.inc - Плотность распределения недренируемых текущих геологических запасов в выбранных k-слоях модели
...
!Текущие объемы нефти в выбранных k-слоях модели, м3                                                  
  ...
  8   continue                                                                                           
      ...
      oiip=oiip*ro/b0                                                                                    
! Недренируемые текущие геологические запасы в выбранных k-слоях модели, кг                            
...
! ndopi1.inc - Плотность распределения недренируемых текущих геологических запасов в выбранных k-слоях модели, кг/м2     
          open(2,file='x.##',form='unformatted')                                                                                
! x.## - Х-координаты центров ячеек                                                                      
          open(3,file='y.##',form='unformatted')                                                                                
! y.## - Y-координаты центров ячеек                                                                      

            if(nvar.eq.1) open(5,file='a-d/ndopi.txt')  
            if(nvar.eq.2) open(5,file='a/ndopi.txt')
            if(nvar.eq.3) open(5,file='b/ndopi.txt')
            if(nvar.eq.4) open(5,file='v/ndopi.txt')
            if(nvar.eq.5) open(5,file='d/ndopi.txt')
! ndopi1.txt - Плотность распределения недренируемых текущих запасов в выбранных k-слоях модели, тыс.т/га
